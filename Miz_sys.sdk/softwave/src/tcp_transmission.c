...
void send_received_data()
{
	...
	{
		Status = XAxiDma_SimpleTransfer(&AxiDma, (u32)RxBufferPtr[0] + HEADER_SIZE,(u32)(ADC_PACKET_LENGTH), XAXIDMA_DEVICE_TO_DMA);
...
int tcp_send_init()
{
	...
	IP4_ADDR(&ipaddress,  192, 168,   1, 209);		/* tcp server address */
